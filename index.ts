...
app.get('/bmi', (req, res) => {
  ...
  if(req.query.weight && req.query.height) {
    try {
      ...
      res.status(200).send({ weightInKg, heightInCm, bmi }).json();
    } catch(err) {
      if (err instanceof Error) res.status(400).send({ error: err.message }).json();
    }
  } else {
    res.status(400).send({ error: 'malformatted perameters' }).json();
  }
});
